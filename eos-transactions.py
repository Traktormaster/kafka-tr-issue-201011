...
"""
import sys  # DEMO-CHANGE: we'll write to stdout stream directly to be in sync with log writes
import time  # DEMO-CHANGE: we'll use this to see what the performance difference is
import argparse
...
def delivery_report(err, msg):
    ...
    """
    if err:
        sys.stdout.write('Message delivery failed ({} [{}]): {}\n'.format(
            msg.topic(), str(msg.partition()), err))  # DEMO-CHANGE
...
def main(args):
    ...
    eof = {}
    msg_cnt = 0
    sys.stdout.write("=== Starting Consume-Transform-Process loop ===\n")  # DEMO-CHANGE
    while True:
        ...
        if msg.error():
            if msg.error().code() == KafkaError._PARTITION_EOF:
                ...
                sys.stdout.write("=== Reached the end of {} [{}] at {}====\n".format(
                    topic, partition, msg.offset()))  # DEMO-CHANGE

                if len(eof) == len(consumer.assignment()):
                    sys.stdout.write("=== Reached end of input ===\n")  # DEMO-CHANGE
                    # break  # DEMO-CHANGE: do not exit, but wait for more messages
            continue
        ...
        if True:  # DEMO-CHANGE: transact every processed message one-by-one
            sys.stdout.write("=== Committing transaction with {} messages at input offset {} ===\n".format(
                msg_cnt, msg.offset()))  # DEMO-CHANGE
            # Send the consumer's position to transaction to commit
            # them along with the transaction, committing both
            # input and outputs in the same transaction is what provides EOS.
            t0 = time.time()  # DEMO-CHANGE
            sys.stdout.write(":DEMO:START {}\n".format(t0))  # DEMO-CHANGE
            producer.send_offsets_to_transaction(
                consumer.position(consumer.assignment()),
                consumer.consumer_group_metadata())
            t1 = time.time()  # DEMO-CHANGE
            sys.stdout.write(":DEMO:END   {} {}\n".format(t1, t1-t0))  # DEMO-CHANGE
